...
const TapLink = ({ options = [] }) => {
  ...
  return (
    <section>
      {options?.map((option) => (
        <article key={option.name} className=" bg-blue-400 flex flex-col relative  items-center justify-center  gap-[18px] w-auto ">
          ...
          <div  className={`flex flex-col gap-4 transition-all  bg-red-600 rounded-sm  overflow-hidden  ${activeLink===option.name ?'max-h-[1000px] delay-200  ease-in-out duration-400':'max-h-0 duration-200 '}` }>
